...
**These rules apply to all UI work (React components, CSS, MUI theme overrides, and any new panels/overlays).**  
...
* **No horizontal scrolling at mobile widths (ever). If something doesn’t fit, it must wrap/truncate.**
...
* **Nested containers may use:**

  * **dividers (1–2px lines), or**

  * **header plates (a small titled slab), but not another “frame.”**
...
**Rule: If two adjacent frames look identical, you must introduce hierarchy via these fills—never via shadows.**
...
* **Use an explicit narrow threshold (e.g. `max-width: 1100px`) for “dock vs sheet”.**

### **Desktop**

* **Right OpsConsole is a dock (does not recenter the page).**
...
* **Command bar wraps into multiple rows.**
...
* **Targeting overlays MUST be opaque. No see-through panels over the board.**
...
**No framer-motion required. CSS transitions/keyframes only.**

### **Animation roles**

* **ROLE A — State advance (phase change): PhaseRuler fill \+ tiny pulse (140ms)**
...
* **If `prefers-reduced-motion: reduce`, disable transforms/pulses; keep minimal opacity changes.**

**Rule: No animation is allowed to move the board or cause layout shift.**
...
* **The component remains dynamic; only the tab/panel is hidden.**
...
* **gradients / glows / soft shadows / transparency overlays for panels**
...
* **“centered modal blob” layouts (content must be docked/structured)**
