Below is a **Codex remediation prompt** that targets exactly what you want: **zig-zag rivers without checkerboards** and **roads without aggressive pruning wipeouts**. It tells Codex to touch **only `terrain.ts`**, keep everything deterministic, and avoid refactors.
...
* “Avoid 2x2 lattice squares during river joins and road routing to prevent checkerboard clusters at intersections.”
