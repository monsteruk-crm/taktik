...
- docs/engine.md  
  Phases, determinism, RNG, invariants, engine API (`startMatch`/`applyIntent`) + event log (terrain params include extra bridge spacing, pathfinding iteration budgets, diagonal-escape movement rule, plus attack queue + per-turn selection limits)
